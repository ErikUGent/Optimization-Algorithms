...
import Windmill_Functions as WmF
...
def click(button) -> None:
    ...
    Args:
        button ([tkinter.Button]): [DThe button that is clicked]
    """
    if (button["bg"] != ACTIVE):
        button["bg"] = ACTIVE
    else:
...
def startKnop(button):
    ...
    Args:
        button ([tkinter.Button]): [The startknop]
    """
        ...
        if COMBOBOXVALUE != None:
            if COMBOBOXVALUE in WmF.windmillObjToNames(resultList):
                sortRoute = WmF.sortBestRoute(WmF.geneticAlgorithm(
                    population=resultList, popSize=100, eliteSize=20, crossoverRate=1, mutationRate=0.01, generations=500), COMBOBOXVALUE)
