...
def breedPopulation(matingpool, eliteSize, crossoverRate):
    ...
    for i in range(0, length):
        child = breed(pool[i], pool[len(matingpool)-i-1], crossoverRate)
...
def nextGeneration(currentGen, eliteSize, crossoverRate, mutationRate):
    ...
    children = breedPopulation(matingpool, eliteSize, crossoverRate) 
...
def jsonToData(file='Json-Data/Windmills.json', datasetName='Windmill_dataset'):
