...
POPULATION_SIZE = 50
CROSSOVER_RATE = 0.9
MUTATION_RATE_TOT = 0.8
MUTATION_RATE = 0.35
NO_OF_MUTATIONS = 7
OVER_WEIGHT_PENALTY = 1000000000000000000000
KEEP_BEST = True

NO_VEHICLES = 4
MAX_VEHICLE_WEIGHT = 650
NO_EXPERIMENT_ITERATIONS = 3
NO_CUSTOMERS = 24
...
def gen_chromosome():
    ...
    for i in range(0, len(_stops)):
        _vehicles.append(random.randint(0, NO_VEHICLES-1))
...
def fun_fitness(costs, weight):
    if weight > 650:
        weight_penalty = (weight - 650) * OVER_WEIGHT_PENALTY
    else:
        weight_penalty = 0
...
def two_opt_one_path(c: Chromosome):
    vehicle_to_check = random.randint(0, NO_VEHICLES-1)
    route = [0]
    for i in range(0, len(c.vehicles)):
...
def two_opt_two_paths(c: Chromosome):
    vehicle_1 = random.randint(0, NO_VEHICLES-1)
    vehicle_2 = random.randint(0, NO_VEHICLES-1)
...
def calculate_map_context():
    ...
    rows[0][2] = np.random.randint(11,41)
    rows[0][3] = np.random.randint(11,41)
    _dist_matrix = []
    _demands = [0] * len(rows)
    rows_all_1.append(rows[0][2])
    rows_all_2.append(rows[0][3])
    print(rows[0])
...
if __name__ == '__main__':
    ...
    for i in range(0, NO_EXPERIMENT_ITERATIONS):
        ...
        optimal_solution_costs = 330
        results = []
        chrom_tot = [Chromosome([], [])]
        j = 0

        #for j in range (0, NO_GENERATIONS):
        if random.uniform(0, 1) < MUTATION_RATE_TOT:
            while j < NO_GENERATIONS:
                distance_matrix, demands, data_matrix = calculate_map_context()
                start_time = time.time()
                chromosome = ga_solve()
                end_time = time.time()
        
                costs_j, weights_j = calculate_path_costs_and_weights(chromosome)
                print_cost_and_weight(costs_j, weights_j, j + 1, end_time - start_time)
                print_phenotype(chromosome)
                total_cpu_time += end_time - start_time
                results.append(sum(costs_j))
                chrom_tot.append(chromosome)
                chrom_tot_tot.append(chromosome)
                results_tot.append(sum(costs_j))
                if abs(results[j] - min(results_tot)) <= 5.0 and results[j] - min(results_tot) != 0.0:
                    break
                j += 1

    final_result_Min = min(results_tot)
    
    ind_fin_res = results_tot.index(final_result_Min) - 1

    final_dep_x = rows_all_1[ind_fin_res] 
    final_dep_y = rows_all_2[ind_fin_res]
    ...
    print("Costs of optimal solution:", "{:.2f}".format(final_result_Min))
    print("Absolute difference of optimal solution:", "{:.2f}".format(final_result_Min - optimal_solution_costs))
    print("Relative difference of optimal solution: ", "{:.2f}".format((100 / optimal_solution_costs * final_result_Min) - 100), "%", sep="")

    print_phenotype(chrom_tot_tot[ind_fin_res])