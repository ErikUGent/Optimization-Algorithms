...
dataset_y = pd.read_csv('Tests_target.csv')

y = dataset_y['Target'].values

# Load dataset and split into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20)

# Train a LightGBM model
model = lgb.LGBMRegressor(importance_type='gain',num_leaves=1000, learning_rate=0.1, n_estimators=60, num_iterations=100)
...
# Train a LightGBM model
model = lgb.LGBMRegressor(importance_type='split', num_leaves=1000, learning_rate=0.1, n_estimators=60, num_iterations=100)
model.fit(X_train, y_train)
