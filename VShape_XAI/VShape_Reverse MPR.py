...
X = dataset[['Number of Birds','Wind Speed','Angle','CW','Target']].values
y = dataset['Surface'].values
...
#sns.regplot(data=X_test, x=X_test[:, 0], y=lin2.predict(poly.fit_transform(X_test)), order=21)
#sns.regplot(data=X_test, x=X_test[:, 1], y=lin2.predict(poly.fit_transform(X_test)), order=2)
sns.regplot(data=X_test, x=X_test[:, 1], y=lin2.predict(poly.fit_transform(X_test)), order=2)
ax.set_xlabel( "Wind Speed" , size = 10)
...
new_data = np.array([[36,-5,180,1.17,72224513]])  # Example values for median income, house age, and average rooms
